import*as React from"react";import styled from"../../styled-components";import{darken}from"polished";import throttle from"lodash.throttle";import{useFullHeight}from"../../hooks";const GithubSlugger=require("github-slugger"),slugger=new GithubSlugger,HEADING_GAP=2;function useActiveHeading(a){const[b,c]=React.useState(void 0),d=React.useMemo(()=>a&&a.querySelectorAll(".hidden-anchor"),[a]),e=React.useCallback(throttle(()=>{if(!d)return;const a=document.body.scrollHeight,b=window.scrollY+window.innerHeight;if(b>=a-HEADING_GAP||2===d.length){const a=d[d.length-1].getAttribute("id")||void 0;return void c(a)}for(let a=0;a<d.length;a++){const b=d[a].parentNode;if(b&&b.offsetTop-HEADING_GAP>window.scrollY){const b=0===a?void 0:d[a-1].getAttribute("id")||void 0;return void c(b)}}}),[d]);return React.useEffect(()=>{if("undefined"!=typeof window&&d&&d.length)return window.addEventListener("scroll",e,{capture:!1}),e(),()=>window.removeEventListener("scroll",e)},[e]),b}export default function TableOfContent(a){const{headings:b,tocMaxDepth:c}=a,d=React.useRef(null);useFullHeight(d);const e=useActiveHeading(a.contentWrapper);return slugger.reset(),b&&1===b.length&&1===b[0]?.depth?null:React.createElement(React.Fragment,null,b&&React.createElement(TableOfContentMenu,null,React.createElement(TableOfContentItems,{ref:d},React.createElement(TableOfContentsHeader,null,"On this page"),b.map(a=>({...a,value:a.value?.replace(/(<([^>]+)>)/gi,"")})).map((a,b)=>{if(0===b&&1===a.depth)return null;if(a.depth&&a.depth>c)return null;const d="#"+(a.id?a.id:slugger.slug(a.value,!1));return React.createElement(MenuItem,{key:d,depth:a.depth||0,href:d,className:"#"+e==d?"active":"",dangerouslySetInnerHTML:{__html:a.value||""},"data-cy":`toc-${a.value}`})}))))}const MenuItem=styled.a`
  display: block;
...
`,TableOfContentsHeader=styled.div`
  padding: 10px 15px;
  font-size: 0.8em;
